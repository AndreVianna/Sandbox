...
public class SimpleReporterTests {
    ...
    public void SimpleReporter_GetDetailedReport_ReturnsReport() {
        ...
        var profiler = factory.CreateProfiler("SomeCategory");
        for (var i = 0; i < 100; i++) {
            profiler.Measure(() => { });
        }

        var reporter = factory.CreateReporter();

        var report = reporter.GetDetailedReport().ToArray();

        report.Should().HaveCount(100);
        report[0].Category.Should().Be("SomeCategory");
        report[0].Action.Should().Be("SimpleReporter_GetDetailedReport_ReturnsReport");
    ...
    public void SimpleProfiler_GetSummaryReport_ReturnsReport() {
        ...
        var profiler = factory.CreateProfiler<SimpleReporterTests>();
        for (var i = 0; i < 100; i++) {
            profiler.Measure(() => { });
        }
        ...
        summary[0].Category.Should().Be("SimpleReporterTests");
        summary[0].Action.Should().Be("SimpleProfiler_GetSummaryReport_ReturnsReport");
